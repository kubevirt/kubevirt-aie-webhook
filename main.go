...
func main() {
	...
	// Create a direct (non-caching) client for VMI lookups in the webhook
	// handler. The manager's default client reads through the cache, which
	...
	// Adding VMIs to the ByObject map is not sufficient either, as the
	// informer is only started when explicitly watched by a controller.
	apiReader, err := client.New(ctrl.GetConfigOrDie(), client.Options{Scheme: scheme})
	if err != nil {
		setupLog.Error(err, "unable to create API reader")
		os.Exit(1)
	}
